...
> 🧊 **Frozen ([PEP-048](https://github.com/typesugar/typesugar/blob/main/peps/PEP-048-package-triage.md)).** `@typesugar/math` is not under active development and is excluded from release. It still lives in the repo and builds, but is not part of typesugar's actively-maintained surface.
...
Matrices with row/column counts tracked at the type level. Dimension mismatches are caught at compile time.
...
`@typesugar/math` re-exports everything from `@typesugar/units`, so you can pull units and math types from a single module:
...
Typeclass instances use `@op` JSDoc tags on method signatures for operator integration. When used with the typesugar macro transform, operators dispatch to the corresponding typeclass methods:
