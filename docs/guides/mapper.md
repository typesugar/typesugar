...
At a glance, `@typesugar/mapper` is:

- **Zero-cost** — All transformations are evaluated at compile time and inlined
- **Type-safe** — Compile-time validation. Fails the build if a target field is not mapped
...
3. `compute` derives target fields from source data
4. `const` provides constant values for fields not in the source
5. Missing mappings cause compile-time errors
...
- Chimney has a richer DSL (`withFieldRenamed`, `withFieldComputed`); `@typesugar/mapper` uses config objects
...
- [Validation guide](/guides/validate) — planned integration target
