...
Compile-time graph algorithms and state machine verification for TypeScript, inspired by Boost.Graph. Define graphs and state machines via a concise DSL or programmatic API, then run classic algorithms (topological sort, shortest path, SCC, cycle detection) and verify state machine properties (reachability, dead ends, nondeterminism) — all with immutable data structures.
...
When used with the typesugar transformer, `stateMachine` validates your state machine at compile time, turning the same structural issues into compile errors:
...
For the `order` machine above, the transformer emits:
