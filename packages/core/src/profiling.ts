...
/**
 * Whether profiling is enabled (check TYPESUGAR_PROFILE env var). `process`
 * doesn't exist in a browser bundle (e.g. the playground) -- this module is
 * shared by both pipelines now, so the check must not assume Node.
 */
export const PROFILING_ENABLED =
  typeof process !== "undefined" && process.env?.TYPESUGAR_PROFILE === "1";
