...
// Specialization pipeline — single shared implementation (PEP-053 Wave 3)
import {
  tryAutoSpecialize as tryAutoSpecializeShared,
...
class MacroTransformer {
  ...
  /**
   * PEP-053 Wave 3: the specialization pipeline (auto-specialization, derived
   * instance inlining, return-type-driven specialization, dead-instance
   * elimination) is ONE shared implementation in @typesugar/transformer-core.
   * These shims map the class state (ctx, verbose, specCache) onto the shared
   * free functions; the former private clone (~700 lines) is deleted.
   */
  private tryAutoSpecialize(node: ts.CallExpression): ts.Expression | undefined {
    return tryAutoSpecializeShared(this.ctx, this.verbose, this.specCache, node);
  }

  /**
   * Inline a direct method call on a known typeclass instance
   * (eqPoint.equals(a, b) -> a.x === b.x && a.y === b.y).
   */
  private tryInlineDerivedInstanceCall(node: ts.CallExpression): ts.Expression | undefined {
    return tryInlineDerivedInstanceCallShared(this.ctx, node, undefined);
  }
  ...
  /**
   * Return-type-driven specialization (Result -> Option/Either/bare-T).
   * PEP-053 Wave 3: delegates to the shared pipeline in
   * @typesugar/transformer-core.
   */
  private tryReturnTypeDrivenSpecialize(node: ts.CallExpression): ts.Expression | undefined {
    return tryReturnTypeDrivenSpecializeShared(this.ctx, this.verbose, this.specCache, node);
  }
...
// Derived-instance DCE lives in @typesugar/transformer-core (PEP-053 Wave 3).
