...
> Functional programming library for TypeScript, inspired by Cats (Scala).

## Overview

`@ttfx/cats` provides a complete functional programming toolkit: typeclasses (Functor, Monad, Applicative), data types (Option, Either, List, Validated), monad transformers (State, Reader, Writer), and an IO monad with stack-safe interpreter.

## Installation

```bash
npm install @ttfx/cats
# or
pnpm add @ttfx/cats
```
...
```typescript
import { Option, Some, None, isSome, isNone } from "@ttfx/cats";
...
```typescript
import { Either, Left, Right, isLeft, isRight } from "@ttfx/cats";
...
```typescript
import { List, Cons, Nil } from "@ttfx/cats";
...
```typescript
import { Validated, valid, invalid, validNel, invalidNel } from "@ttfx/cats";
...
```typescript
import { IO, runIO, runIOSync } from "@ttfx/cats";
...
```typescript
import { State, IndexedState } from "@ttfx/cats";
...
```typescript
import { Reader, Kleisli } from "@ttfx/cats";
...
```typescript
import { Writer, LogWriter } from "@ttfx/cats";
...
See also: `@ttfx/cats/zero-cost` for direct imports.

## Syntax Utilities

```typescript
import { pipe, flow } from "@ttfx/cats";
