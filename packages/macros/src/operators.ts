 ...
 * enhancement could serialize mappings to a .typesugar-cache file.
 */
...
export const operatorMethodAttribute = defineAttributeMacro({
  ...
  expand(
    ...
    if (!ts.isMethodDeclaration(target) || !target.name) {
      ctx.reportError(decorator, "@operator can only be applied to named methods");
      return target;
    }

    // Get the operator symbol from the decorator argument
    if (args.length !== 1 || !ts.isStringLiteral(args[0])) {
      ctx.reportError(decorator, '@operator requires a string argument (e.g., @operator("==="))');
      return target;
    }
...
export const binopMacro = defineExpressionMacro({
  name: "__binop__",
  // No module — this is a global macro generated by the preprocessor, not user-imported
  ...
  expand(
    ...
    if (args.length !== 3) {
      ctx.reportError(callExpr, "__binop__ expects exactly 3 arguments: (left, operator, right)");
      return callExpr;
    }
    ...
    // Get the type of the left operand (may be undefined in some contexts)
    // Wrapped in try-catch because type resolution can fail on synthetic nodes
    let baseTypeName = "";
    try {
      ...
      // Type resolution failed - proceed with fallback semantics
    }
