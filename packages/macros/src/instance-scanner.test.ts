...
describe("resolveTypeString keyword resolution (PEP-052 Wave 5 review fix)", () => {
  ...
  it("does not conflate `symbol`, `unknown`, `object`, and `number` typed instances with each other", () => {
    const mod = createTestModule(`
/** @impl Ord<symbol> */
...
/** @impl Ord<object> */
export const ordObject = { compare: (a: object, b: object) => 0 };
/** @impl Ord<number> */
export const ordNumber = { compare: (a: number, b: number) => 0 };
    ...
    // Every instance must resolve to a DEFINED forType — not just "not Any".
    // A keyword silently regressing to `undefined` (e.g. an internal
    // TypeChecker getter disappearing in a future TS version, falling into
    // the any-guard's undefined return) would pass a weaker "falsy-Any"
    // check while the instance quietly becomes unresolvable in production.
    for (const r of results) {
      expect(r.forType, `${r.exportName} must resolve to a concrete type`).toBeDefined();
      expect(r.forType!.flags & ts.TypeFlags.Any).toBe(0);
    }
    // And each instance's resolved forType must be distinct — none should
    // have silently collapsed to `any` or to each other.
    const forTypes = new Set(results.map((r) => bySymbolName.get(r.exportName)!.forType));
    expect(forTypes.size).toBe(4);
  });
});
