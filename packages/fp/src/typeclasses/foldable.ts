...
export function foldMap<F>(
  F: Foldable<F>
): <M>(M: Monoid<M>) => <A>(fa: Kind<F, A>, f: (a: A) => M) => M {
 ...
 */
export function fold<F>(F: Foldable<F>): <A>(M: Monoid<A>) => (fa: Kind<F, A>) => A {
 ...
 */
export function exists<F>(F: Foldable<F>): <A>(fa: Kind<F, A>, p: (a: A) => boolean) => boolean {
 ...
 */
export function forall<F>(F: Foldable<F>): <A>(fa: Kind<F, A>, p: (a: A) => boolean) => boolean {
...
export function last<F>(F: Foldable<F>): <A>(fa: Kind<F, A>) => A | undefined {
  return <A>(fa: Kind<F, A>): A | undefined =>
 ...
 */
export function filter<F>(F: Foldable<F>): <A>(fa: Kind<F, A>, p: (a: A) => boolean) => A[] {
  return <A>(fa: Kind<F, A>, p: (a: A) => boolean): A[] =>
    F.foldLeft<A, A[]>(fa, [], (acc, a) => {
      if (p(a)) acc.push(a);
      return acc;
    });
...
export function minimum<F>(
  ...
  return <A>(fa: Kind<F, A>, compare: (a: A, b: A) => number): A | undefined =>
    F.foldLeft<A, A | undefined>(fa, undefined, (acc, a) =>
