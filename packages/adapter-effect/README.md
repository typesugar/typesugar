...
`@ttfx/adapter-effect` provides seamless integration between ttfx macros and Effect-TS. Use labeled block syntax or expression macros to write Effect code with cleaner syntax that compiles to standard Effect API calls.

## Installation

```bash
npm install @ttfx/adapter-effect
# or
pnpm add @ttfx/adapter-effect
```
...
```typescript
import { gen$ } from "@ttfx/adapter-effect";
...
```typescript
import { map$ } from "@ttfx/adapter-effect";
...
```typescript
import { flatMap$ } from "@ttfx/adapter-effect";
...
```typescript
import { pipe$ } from "@ttfx/adapter-effect";
...
- `let: { ... } yield: { ... }` — Do-notation with `<<` binding syntax
...
- `Bind<T>` — Marker type for `<<` binding expressions
