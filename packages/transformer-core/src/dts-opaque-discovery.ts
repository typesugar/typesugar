...
// ---------------------------------------------------------------------------
// File access — injected so this module has no hard Node dependency
// ---------------------------------------------------------------------------

/**
 * Minimal file-access surface `resolveRelativeDts`'s disk fallback needs. Node
 * callers get real disk access via `defaultFileAccess`; browser callers (this
 * module is shared with `transformer-core`'s browser bundle) can pass their
 * own in-memory implementation, or omit it -- files already in `program`
 * still resolve normally either way, only the disk-fallback path is affected.
 */
export interface DtsFileAccess {
  fileExists: (path: string) => boolean;
  readFile: (path: string) => string | undefined;
}

const defaultFileAccess: DtsFileAccess = {
  fileExists: (p) => (typeof ts.sys !== "undefined" ? ts.sys.fileExists(p) : false),
  readFile: (p) => (typeof ts.sys !== "undefined" ? ts.sys.readFile(p) : undefined),
};
 ...
 * @param fileAccess - Disk access for `.d.ts` files not already in `program`
 *   (e.g. transitively re-exported files). Defaults to `ts.sys` when available.
 */
export function discoverOpaqueTypesFromImports(
  sourceFile: ts.SourceFile,
  program: ts.Program,
  verbose?: boolean,
  fileAccess: DtsFileAccess = defaultFileAccess
): void {
  ...
  for (const stmt of sourceFile.statements) {
    ...
    const collected = collectReExportedDtsFiles(dtsSourceFile, program, fileAccess);
 ...
 */
function collectReExportedDtsFiles(
  entry: ts.SourceFile,
  program: ts.Program,
  fileAccess: DtsFileAccess
): ts.SourceFile[] {
  ...
  while (stack.length > 0) {
    ...
    for (const stmt of file.statements) {
      ...
      const targetFile = resolveRelativeDts(
        file.fileName,
 ...
 *
 * `fileAccess` is injected rather than reaching for `ts.sys` directly -- `ts.sys`
 * doesn't exist in a browser bundle (e.g. the playground), which is why this
 * function used to be Node-only. Node callers get real disk access via
 * `defaultFileAccess` (below); browser callers that don't pass one simply don't
 * get this fallback (files already in `program` still resolve normally).
 */
function resolveRelativeDts(
  ...
  for (const candidate of candidates) {
    ...
    if (fileAccess.fileExists(resolved)) {
      const text = fileAccess.readFile(resolved);
      if (text !== undefined) {
