...
15. **Diagnostic Suppression Rules (PEP-011, renamed by PEP-054)** — When TypeScript would report errors at type rewrite boundaries (e.g., assigning `T | null` to `Option<T>`), diagnostic suppression rules suppress the diagnostic because the types are runtime-identical. Three rules: (1) TypeRewriteAssignment — assignment/argument passing between `@opaque` type and its underlying; (2) MethodOnUnderlyingType — calling a method on the underlying representation that exists on the opaque interface; (3) PropertyAccessOnUnderlyingType — accessing properties that exist on the opaque interface. All suppressions are logged and can be reviewed via `--show-suppressed-diagnostics` (audit mode).

16. **Type Rewrite Registry** — Central registry in `@typesugar/core` that maps opaque type names to their underlying representation, companion functions, constructors, and accessors. Populated by `@opaque` macros, consulted by the transformer (for method erasure), diagnostic suppression rules (for implicit conversions), and the language service plugin (for completions). Lookup by type name, type symbol, or source module.

17. **Global Augmentation for Extension Methods** — `@typesugar/std` uses `declare global { interface Number { ... } }` to add type-checked methods to built-in types. The transformer still rewrites these to function calls (no prototype mutation). Augmented: Number, String, Array, Map, Promise, Date, Boolean. Not augmented: Set (ES2025 conflicts), Object, Function (too broad).
...
| Register an @opaque type        | `/** @opaque UnderlyingType */` on interface                            | `packages/macros/src/opaque.ts`               |
| Look up type rewrite entry      | `typeRewriteRegistry.get(typeName)`                                     | `packages/core/src/type-rewrite-registry.ts`  |
| Register type rewrite           | `typeRewriteRegistry.register(entry)`                                   | `packages/core/src/type-rewrite-registry.ts`  |
| Check diagnostic suppression    | `evaluateDiagnosticSuppression(diagnostic, checker, sourceFile)`        | `packages/core/src/diagnostic-suppression.ts` |
