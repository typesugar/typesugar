...
/**
 * Decorator to mark a method as implementing a custom operator.
 * Used with the preprocessor's custom operator syntax (|>, ::, etc.).
 *
 * @param symbol - The operator symbol this method implements
 *
 * @example
 * ```typescript
 * class Stream<A> {
 *   @operator('|>')
 *   pipe<B>(f: (s: Stream<A>) => Stream<B>): Stream<B> { ... }
 *
 *   @operator('::')
 *   prepend(head: A): Stream<A> { ... }
 * }
 *
 * // Usage with preprocessor syntax:
 * stream |> transform |> collect
 * head :: tail
 * ```
 */
export function operator(_symbol: string): MethodDecorator {
  // Placeholder decorator - processed by transformer
  return () => {};
}

/**
 * Binary operator dispatch function.
 * Generated by the preprocessor for custom operators (|>, ::, etc.).
 *
 * The macro transformer resolves this based on the left operand's type:
 * - If the type has @operator(symbol) → rewrite to method call
 * - Otherwise, falls back to default behavior per operator
 *
 * **Default fallbacks:**
 * - `|>` (pipeline): `f(left)` - standard pipeline semantics
 * - `::` (cons): `[left, ...right]` - array cons
 * - `<|` (reverse pipeline): `left(right)`
 *
 ...
 * x |> f |> g
 * // To:
 * __binop__(__binop__(x, "|>", f), "|>", g)
 *
 * // Which the transformer resolves to:
 * g(f(x))  // or x.pipe(f).pipe(g) if pipe method exists
 * ```
 */
export function __binop__<T>(left: T, op: string, right: unknown): unknown {
  // Runtime fallback - used when transformer doesn't process the code
  switch (op) {
    case "|>":
      return (right as (arg: T) => unknown)(left);
    case "::":
      return [left, ...(right as unknown[])];
    case "<|":
      return (left as (arg: unknown) => unknown)(right);
    default:
      throw new Error(`Unknown custom operator: ${op}`);
  }
}
 ...
 *
 * @param typeName - The type to extend (e.g., "number", "string", "Date")
 * @param fn - The function (its name becomes the method name)
 *
 * @example
 * ```typescript
 ...
 * ```
 */
export function registerExtension(_typeName: string, _fn: Function): void {
  // Placeholder - processed by transformer at compile time
}
