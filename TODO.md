...
- [x] **VSCode Extension** -- Full IDE extension at `packages/vscode-ttfx/` (`@ttfx/vscode-ttfx`). Manifest-driven architecture: all macro names come from `ttfx.manifest.json` (generated by `ttfx build --manifest`), so adding new macros automatically gets IDE support. Features:
...
- [x] **Test coverage gaps** — Added comprehensive tests:
  ...
  - Remaining gaps: `packages/comptime/` edge cases, `packages/cats/src/effect/` Effect types, `packages/vscode-ttfx/` extension tests, unplugin adapter integration tests, `packages/react/` and `packages/testing/` tests
...
- [ ] **Specialization Ergonomics** -- Three levels of compile-time specialization for typeclass-based generic programming:
  ...
  **Level 2: Scoped Specialization** (design for later)
  ...
  import { optionMonad } from "@ttfx/cats" with { implicit: true };
  ```
...
- [ ] **`Eval` monad for cats library** -- Add a trampolined lazy evaluation monad (`Eval<A>`) to `@ttfx/cats`. **Not needed for List operations** (those were converted to iterative while loops), but valuable for:
...
- Typeclasses (`Functor`, `Monad`, etc.) moved to `@ttfx/cats` package
...
3. The `@ttfx/cats` `match` macro already handles discriminated union matching better
...
`packages/effect-do/`. Tests in `tests/comprehensions.test.ts` (51 tests).
...
- [ ] **Monorepo publish** — Publish all packages: `ttfx`, `@ttfx/core`, `@ttfx/cats`, etc.
